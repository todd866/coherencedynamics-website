...
In fact, there's a strong case that the fields *preceded* the genes. The first protocells were lipid bilayers---low-dimensional membranes that created electrostatic boundaries. But the "thinking" happened in the field: the high-dimensional phase space of ion gradients, charge distributions, and chemical potentials both inside and outside that membrane. The membrane steered; the field computed. Inner and outer, self and environment, coupled through a low-dimensional interface. Genetic codes came later, as a way to *stabilize* and *transmit* successful configurations. The low-D structure didn't create the high-D dynamics; it learned to ride them.
